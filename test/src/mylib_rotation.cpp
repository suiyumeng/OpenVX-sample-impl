...
vx_status vx_test_mylib_rotation(int argc, char *argv[])
{
    ...
    if (vxGetStatus((vx_reference)context) == VX_SUCCESS)
    {
        ...
        if (vxGetStatus((vx_reference)graph) == VX_SUCCESS)
        {
            ...
            status = vxLoadKernels(context, "openvx-mylib");
            node = vxMyRotationNode(graph, images[0], VX_ROTATION_180, images[1]);
            if (node)
            {
                ...
                if (status == VX_SUCCESS)
                {
                    ...
                    if (status == VX_SUCCESS) {
                        ...
                        if (status == VX_SUCCESS) {
                            ...
                            cv::Mat img_out(h, w, CV_8UC3, ptr_out); // CV_8UC3表示3通道8位无符号整数
            ...
            status |= vxUnloadKernels(context, "openvx-mylib");
        }
        else
        {
