...
#include "vx_mylib_module.h"

static vx_kernel_description_t* kernels[] =
{
    &mylib_rotation_kernel,
};
...
/*VX_API_ENTRY*/ vx_status VX_API_CALL vxUnpublishKernels(vx_context context)
{
    ...
    for (k = 0; k < num_kernels; k++)
    {
        //vx_kernel kernel = vxGetKernelByName(context, kernels[k]->name); //[vxGetKernelByName:372] Kernel name should not contain any ':' in this implementation  ??
        vx_kernel kernel = vxGetKernelByEnum(context, kernels[k]->enumeration);
